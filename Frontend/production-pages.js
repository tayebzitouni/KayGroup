(function installKayProductionPages(){
  ...
  function dashboard(){
    ...
    return `${head('Tableau de bord','La situation consolidée du Groupe, calculée depuis le Transaction Engine.',button('Nouvelle opération','new-operation','primary','plus')+button('Actualiser','refresh','secondary','refresh'))}
      ...
      <section class="grid kp-dashboard-grid"><article class="panel kp-chart"><div class="panel-head"><div><h2>Activité mensuelle</h2><p>Ventes et coûts issus des opérations enregistrées</p></div><div class="kp-legend"><span><i></i>Ventes</span><span><i></i>Coûts</span></div></div>${series.length?`<div class="kp-bars">${series.map(item=>`<div class="kp-bar-group"><div class="kp-bar-pair"><i class="sales" style="height:${Math.max(3,item.sales/maximum*100)}%" title="Ventes ${money(item.sales)}"></i><i class="costs" style="height:${Math.max(3,item.costs/maximum*100)}%" title="Coûts ${money(item.costs)}"></i></div><span>${esc(item.label)}</span></div>`).join('')}</div>`:empty('Aucune activité datée','Les graphiques se rempliront après les premières opérations.')}</article>
      <article class="panel kp-aging"><div class="panel-head"><div><h2>Balance âgée clients</h2><p>Répartition de l’encours réel</p></div><button type="button" class="text-link" data-kp-action="navigate" data-page="aged">Voir le détail →</button></div><div class="kp-aging-body"><div class="kp-donut" style="--a:${ageValues[0]/ageTotal*360}deg;--b:${(ageValues[0]+ageValues[1])/ageTotal*360}deg;--c:${(ageValues[0]+ageValues[1]+ageValues[2])/ageTotal*360}deg"><div><strong>${money(ageValues.reduce((a,b)=>a+b,0))}</strong><span>encours</span></div></div><ul>${[['Non échue',ageValues[0]],['1–30 jours',ageValues[1]],['31–90 jours',ageValues[2]],['Plus de 90 jours',ageValues[3]]].map(([label,value],index)=>`<li class="age-${index}"><i></i><span>${label}</span><strong>${money(value)}</strong></li>`).join('')}</ul></div></article></section>
  ...
  function administration(){
    ...
    return `${head('Administration','Référentiel Groupe, utilisateurs, rôles et paramètres du moteur.',button('Référentiel Groupe','navigate','primary','building','data-page="masterData"')+button('Sécurité & rôles','navigate','secondary','settings','data-page="security"'))}<section class="grid kp-domain-kpis">${kpi('Sociétés',String(companies.length),`${sites.length} site${sites.length===1?'':'s'}`,'building')}${kpi('Laboratoires',String(labs.length),`${centers.length} centre${centers.length===1?'':'s'} de coût`,'journal')}${kpi('Utilisateurs actifs',String(activeUsers.length),`${roles.length} rôle${roles.length===1?'':'s'}`,'users')}${kpi('MFA obligatoire',source.raw?.settings?.requireMfa?'Oui':'Non','Paramètre de sécurité','shield',source.raw?.settings?.requireMfa?'':'warning')}</section>
  ...
  window.runProductionDomainChecks=()=>{const original=state.page,checks={};try{for(const route of routes){navigate(route);const root=document.querySelector('#app-content'),buttons=[...root.querySelectorAll('button')];checks[`${route}_title`]=root.querySelector('h1')?.textContent.trim()===titles[route];checks[`${route}_bound`]=root.dataset.productionPage===route;checks[`${route}_data_source`]=root.querySelector('.eyebrow')?.textContent.includes('TEMPS RÉEL');checks[`${route}_buttons`]=buttons.every(item=>Boolean(item.dataset.kpAction)||item.type==='submit')}}finally{navigate(original)}const values=Object.values(checks);return{ok:values.every(Boolean),total:values.length,passed:values.filter(Boolean).length,checks}};
  ...
  document.addEventListener('click',event=>{
    ...
    if(button.id==='user-menu-button'){
      ...
      showPopover(button,'Compte actif',[{icon:'U',title:current.displayName||'Utilisateur KAY ONE',text:current.email||'Session locale'},{icon:'MFA',title:'Authentification renforcée',text:current.mfaEnabled?'MFA configurée':'MFA à configurer'},{icon:'⚙',title:'Habilitations',text:'Gérées dans Sécurité & rôles'}]);return;
    }
  },true);
  renderProduction();
})();
