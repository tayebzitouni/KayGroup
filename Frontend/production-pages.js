(function installKayProductionPages(){
  ...
  const isCostType=value=>typeIsNormalized(value,'achat|import|decaissement|note de frais|note\\s+de\\s+frais|fourn|banque|caisse|fiscalit|operation diverse|opération diverse|opération|divers|paie|immobilisation|service|frais|taxe');
  ...
  function monthSeries(source){
    const operations=arr(raw(source).operations||source.operations),activeIds=new Set(operations.filter(item=>!/(draft|brouillon|submitted|à valider|a valider|cancel|annul)/i.test(String(item.status||''))).map(item=>String(item.id||item.operationId)));
    const facts=arr(source.reportingFacts||raw(source).reportingFacts).filter(item=>!/(cancel|superseded|annul)/i.test(item.status||'')&&(!activeIds.size||activeIds.has(String(item.operationId))));
    if(facts.length){
  ...
  function dashboard(){
    ...
    const barHeight=value=>num(value)>0?Math.max(3,num(value)/maximum*100):0;
