(function installKayAuthentication(){
  'use strict';
  const state={mode:'loading',email:'',user:null,pendingEmail:'',message:'',enrollment:null};
  ...
  function render(){
    ...
    if(state.mode==='mfa'||state.mode==='enrollment'){
      const enrollment=state.mode==='enrollment'&&state.enrollment?`<div class="kay-auth-enrollment"><p>1. Dans Microsoft Authenticator, Google Authenticator ou 1Password, ajoutez une clé de configuration.</p><button type="button" data-auth-action="copy-secret" data-secret="${esc(state.enrollment.secret)}"><span>${esc(state.enrollment.secret)}</span><strong>Copier</strong></button><small>Conservez cette clé dans un lieu sûr. Elle ne sera plus affichée.</small><p>2. Saisissez ensuite le code à 6 chiffres généré par l’application.</p></div>`:'';
      layer.innerHTML=`<section class="kay-auth-card kay-auth-compact">${logo()}<header><span>DOUBLE AUTHENTIFICATION</span><h1>${state.mode==='enrollment'?'Activer la MFA':'Code de sécurité'}</h1><p>${state.mode==='enrollment'?'Cette installation impose un second facteur avant d’ouvrir la première session.':'Saisissez le code temporaire à 6 chiffres de votre application d’authentification.'}</p></header>${enrollment}<form data-auth-form="mfa"><label><span>Code à usage unique</span><input name="code" class="kay-auth-code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" required autofocus></label>${message()}<button type="submit" class="primary-button">Vérifier le code</button><button type="button" class="kay-auth-link" data-auth-action="back-login">Retour à la connexion</button></form></section>`;return
    }
  ...
  function submit(event){
    ...
    if(form.dataset.authForm==='mfa'){post('verify-mfa',{email:state.pendingEmail,code:payload.code});disable(form,'Vérification…')}
  }
  ...
  function openAccount(){
    ...
    const user=state.user||{};account.innerHTML=`<section class="kay-account-card" role="dialog" aria-modal="true"><header><div class="avatar">${esc(String(user.displayName||'KO').split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</div><div><strong>${esc(user.displayName||'Utilisateur KAY ONE')}</strong><span>${esc(user.email||'')}</span></div><button type="button" data-auth-action="close-account" aria-label="Fermer">×</button></header><div class="kay-account-security"><span>Session protégée</span><strong>${user.mfaEnabled?'Mot de passe + MFA':'Mot de passe'}</strong></div><div class="kay-account-actions"><button type="button" class="secondary-button" data-auth-action="security">Sécurité & rôles</button><button type="button" class="kay-auth-logout" data-auth-action="logout">Se déconnecter</button></div></section>`;document.body.appendChild(account);
  }
  document.addEventListener('submit',submit,true);
  document.addEventListener('click',event=>{const control=event.target.closest('[data-auth-action]'),action=control?.dataset.authAction;if(!action)return;event.preventDefault();event.stopImmediatePropagation();if(action==='back-login'){state.enrollment=null;setMode('login',{email:state.pendingEmail})}if(action==='copy-secret'){navigator.clipboard?.writeText(control.dataset.secret||'');control.querySelector('strong').textContent='Copié'}if(action==='close-account')event.target.closest('.kay-account-layer')?.remove();if(action==='security'){event.target.closest('.kay-account-layer')?.remove();navigate('security')}if(action==='logout'){event.target.closest('.kay-account-layer')?.remove();post('logout');setMode('loading')}},true);
  if(window.chrome?.webview&&typeof window.chrome.webview.addEventListener==='function')window.chrome.webview.addEventListener('message',event=>{
    ...
    if(message.type==='auth-mfa-enrollment'){state.pendingEmail=message.email||state.pendingEmail;state.enrollment={secret:message.secret||'',otpAuthUri:message.otpAuthUri||''};setMode('enrollment')}
    if(message.type==='auth-mfa-required'){state.pendingEmail=message.email||state.pendingEmail;setMode('mfa',{message:message.message||''})}
    if(message.type==='auth-error')setMode(message.stage==='mfa'?'mfa':message.setup?'setup':'login',{email:message.email||state.pendingEmail||state.email,message:message.message||'Authentification impossible.'});
