(function () {
  ...
  function renderCashBoxes(data = {}) {
    ...
    const moveRows = moves.map(item => `<tr data-ka-row data-date="${esc(read(item, 'movementDate', 'date'))}" data-amount="${Math.abs(numeric(read(item, 'amountMad', 'amount')))}"><td>${shortDate(read(item, 'movementDate', 'date'))}</td><td class="ref">${esc(ref(item))}</td><td>${esc(read(item, 'label', 'description', 'kind'))}</td><td>${esc(read(item, 'cashBoxName', 'cashBoxId'))}</td><td class="amount ${/out|sortie|décaissement/i.test(read(item, 'direction')) ? 'negative' : 'ka-positive-text'}">${money(read(item, 'amountMad', 'amount'))}</td><td>${badge(read(item, 'status') || 'Comptabilisé')}</td><td>${iconButton('open-cash-movement', `Ouvrir ${ref(item)}`, 'more', `data-record="${esc(ref(item))}"`)}</td></tr>`).join('');
  ...
  function bind(page, data = {}, send) {
    ...
    root.addEventListener('click', event => {
      ...
      if (contract.mode === 'local+dispatch') emit(send, action, { page, record, domain });
    ...
    root.addEventListener('submit', event => {
      ...
      else payload = { ...Object.fromEntries(new FormData(form).entries()), kind: form.dataset.kind || '', record: submitter?.dataset.record || '' };
